...
from .sequencer_device import add_sequencers, SequencerInfo
...
class QsUploader:
    ...
    def __init__(self, awg_devices, awg_channels, marker_channels,
                 IQ_channels, digitizers, digitizer_channels):
    ...
    def add_upload_job(self, job):
        ...
        aggregator = UploadAggregator(self.AWGs, self.awg_channels, self.marker_channels, self.qubit_channels,
                                      self.sequencer_channels, self.sequencer_out_channels)
    ...
    def play(self, seq_id, index, release_job = True):
        ...
        for qubit_channel in self.qubit_channels.values():
            ...
            for number,wvf in enumerate(job.sequencer_waveforms[qubit_channel.channel_name]):
                seq.upload_waveform(number, wvf.amplitude*wvf.envelope,
...
class Waveform:
    wvf_id: str
    amplitude: float = 0
    envelope: Union[float, np.ndarray, None] = None
    prephase: float = 0
    postphase: float = 0
    duration: int = 0
    clock_offset: int = 0 # offset on 10 ns boundary

    def __eq__(self, other):
        return (self.wvf_id == other.wvf_id
                and self.amplitude == other.amplitude
                and np.all(self.envelope == other.envelope)
...
class UploadAggregator:
    ...
    def __init__(self, AWGs, awg_channels, marker_channels, qubit_channels, sequencer_channels, sequencer_out_channels):
        self.AWGs = AWGs
        self.npt = 0
        self.marker_channels = marker_channels
    ...
    def _generate_upload(self, job, awg_upload_func):
        ...
        for channel_name, channel_info in self.channels.items():
            if (channel_name in self.sequencer_channels
...
#                    n_delay_welding = round(channel_info.delay_ns * section.sample_rate)
    ...
    def _render_waveform(self, waveforms:List[Waveform], mw_pulse_data) -> int:
        # always rendering at 1e9 Sa/s
        duration = mw_pulse_data.stop - mw_pulse_data.start
        offset = int(mw_pulse_data.start / 10) * 10
        if mw_pulse_data.envelope is None:
            amp_envelope = 1.0
        else:
            amp_envelope = mw_pulse_data.envelope.get_AM_envelope(duration, 1.0)
            if offset > 0:
                amp_envelope = np.concatenate([np.zeros(offset), amp_envelope])

        # TODO @@@ phase_envelope; frequency when it differs from qubit frequency
    ...
    def _generate_sequencer_upload(self, job):
        ...
        for channel_name in self.qubit_channels:
            waveforms = []
            sequence = []
            t_start = 0
            entry = SequenceEntry()
            ...
            for iseg,(seg,seg_render) in enumerate(zip(job.sequence,segments)):
                ...
                for mw_pulse_data in data.MW_pulse_data:
                    t_pulse = seg_render.t_start + int(mw_pulse_data.start / 10) * 10
                    ...
                    entry.waveform_index = self._render_waveform(waveforms, mw_pulse_data)
                    t_start = t_pulse
